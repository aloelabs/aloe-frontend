...
function findNearestElementByTime<T extends Timestamp>(
  arr: T[],
  timestamp: string | number
): T {
  const costFunc = arr.map((el) =>
    Math.abs(Number(el.timestamp) - Number(timestamp))
  );
  const idx = costFunc.reduce(
    (iMin, cost, i, arr) => (cost < arr[iMin] ? i : iMin),
    0
  );
  return arr[idx];
}
...
export default function PoolPositionWidget(props: PoolPositionWidgetProps) {
  const { walletIsConnected, poolData, offChainPoolStats, accountAddress } =
    props;
  ...
  useEffect(() => {
    ...
    async function fetchAccountStats(
      ...
      const accountValue = tvl
        .mul(toBig(accountShareBalance.value))
      ...
      const getPoolReturns = axios.get(
        `${API_URL}/pool_returns/${poolData.poolAddress}/1/all/${endTime}`
      );
      const getPrices0 = axios.get(
        `${API_URL}/token_returns/${poolData.token0Address}/1/all/${endTime}`
      );
      const getPrices1 = axios.get(
        `${API_URL}/token_returns/${poolData.token1Address}/1/all/${endTime}`
      );
      ...
      const getPoolReturns3M = axios.get(
        `${API_URL}/pool_returns/${poolData.poolAddress}/1/3m/${endTime}`
      );
      const getPrices03M = axios.get(
        `${API_URL}/token_returns/${poolData.token0Address}/1/3m/${endTime}`
      );
      const getPrices13M = axios.get(
        `${API_URL}/token_returns/${poolData.token1Address}/1/3m/${endTime}`
      );
        ...
        .then(
          axios.spread(
            (
              ...
              let poolReturns = (poolReturnsData.data as PoolReturns).concat(
                ...(poolReturnsData3M.data as PoolReturns)
              );
              poolReturns = poolReturns.filter((r) => r.total_supply !== null);
              const prices0 = (price0Data.data as TokenReturns).concat(
                ...(price0Data3M.data as TokenReturns)
              );
              const prices1 = (price1Data.data as TokenReturns).concat(
                ...(price1Data3M.data as TokenReturns)
              );
              ...
              for (const transfer of transfers) {
                const addrFrom: string = transfer.topics[1]
                  .slice(26)
                  .toLowerCase();
                const addrTo: string = transfer.topics[2]
                  .slice(26)
                  .toLowerCase();
                const amountShares = toBig(BigNumber.from(transfer.data)).div(
                  '1e18'
                );
                ...
                const R = findNearestElementByTime(
                  poolReturns,
                  transfer.timeStamp
                );
                const P0 = findNearestElementByTime(
                  prices0,
                  transfer.timeStamp
                );
                const P1 = findNearestElementByTime(
                  prices1,
                  transfer.timeStamp
                );

                const valueToken0 = amountShares
                  .mul(R.inventory0)
                  .div(R.total_supply)
                  .mul(P0.price);
                const valueToken1 = amountShares
                  .mul(R.inventory1)
                  .div(R.total_supply)
                  .mul(P1.price);
                const valueAll =
                  valueToken0.toNumber() + valueToken1.toNumber();

                const isWithin1D =
                  Number(transfer.timeStamp) > twentyFourHoursAgo;
              ...
              let accountValue1DAgo: number | null = null;
              {
                ...
                const accountShares1DAgo = toBig(accountShareBalance.value)
                  .div('1e18')
                  .minus(shareDiff1D);
                const valueToken0 = accountShares1DAgo
                  .mul(R.inventory0)
                  .div(R.total_supply)
                  .mul(P0.price);
                const valueToken1 = accountShares1DAgo
                  .mul(R.inventory1)
                  .div(R.total_supply)
                  .mul(P1.price);
                accountValue1DAgo =
              ...
              if (mounted) {
                setAccountStats({
                  ...
                  accountValue1DAgo,
                  todaysReturn:
                    accountValue.toNumber() - cashBasis1D - accountValue1DAgo,
  ...
  return (
    <Wrapper>
      ...
      <PerformanceCardGrid>
        <PerformanceCard>
          <Text size='M' weight='medium' color={PERFORMANCE_LABEL_TEXT_COLOR}>
            Today's Return
          </Text>
          <div className='flex items-center gap-4'>
            <Display
              size='L'
              weight='semibold'
              color={PERFORMANCE_VALUE_TEXT_COLOR}
            >
            ...
            <PercentChange
              percent={
                accountStats &&
                accountStats.todaysReturn &&
                accountStats.accountValue1DAgo
                  ? 100 *
                    (accountStats.todaysReturn / accountStats.accountValue1DAgo)
                  : 0.0
              }
            />
          </div>

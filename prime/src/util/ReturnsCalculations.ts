...
export function calculateReturns(poolReturns: PoolReturns, token0Returns: TokenReturns, token1Returns: TokenReturns) {
  ...
  const initialIndex = containsNull
    ? poolReturnsCopy.length - poolReturnsCopy.reverse().findIndex((r) => r.total_supply === null)
    : 0;
  ...
  const initialPoolTVL =
    poolReturns[initialIndex].inventory0 * initialP0 + poolReturns[initialIndex].inventory1 * initialP1;

...
export async function createMarginAccount(
  ...
  const factory = new Contract(
    ALOE_II_FACTORY_ADDRESS_BUILT_IN_FAUCET_GOERLI,
    FactoryABI,
    signer
  );
  ...
  try {
    const estimatedGas = (
      (await factory.estimateGas.createMarginAccount(
        poolAddress,
        ownerAddress
      )) as BigNumber
    ).toNumber();
  ...
  try {
    const transactionResponse = await factory.createMarginAccount(
      poolAddress,
      ownerAddress,
      transactionOptions
    );

...
export default function TokenDropdown(props: TokenDropdownProps) {
  const { options, selectedOption, onSelect, size, backgroundColor, backgroundColorHover, compact, placeAbove } = props;
    ...
    <DropdownWrapper ref={dropdownRef} compact={compact}>
      ...
      >
        <div className='flex items-center gap-1'>
          <TokenIcon src={selectedOption.iconPath || ''} width={20} height={20} />
      ...
      {isOpen && (
        <DropdownList backgroundColor={backgroundColor}>
          {options.map((option) => (
            ...
            >
              <div className='flex items-center gap-1'>
                <TokenIcon src={option.iconPath || ''} width={20} height={20} />
                <Text size='M' weight='bold'>
                  {option.ticker}
                </Text>
